...
def readShortVideo(video_path, video_category, video_name, downsample_factor=12, rescale_factor=1):
    ...
    '''
    skvideo.setFFmpegPath('D:/ffmpeg-4.2.1-win64-static/bin/')
    ...
    for frameIdx, frame in enumerate(videogen):
        if frameIdx % downsample_factor == 0:
            ...
            #frame = skimage.transform.resize(frame, (200, 200)).astype(np.uint8)
            #frames.append(np.moveaxis(frame, 2, 0))
            frames.append(normalize(frame).numpy().astype(np.float))
        else:
            continue

    return np.array(frames).astype(np.float)
